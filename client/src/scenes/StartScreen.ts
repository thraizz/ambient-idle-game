...
export default class StartScreen extends Phaser.Scene {
  ...
  async create(): Promise<void> {
    ...
    if (playerData) {
      try {
        ...
        await this.syncPlayerData(parsedPlayerData);
    ...
    // Add title text
    this.add
      .text(400, 150, "Ambient Idle Game", {
        fontSize: "48px",
        fontFamily: "Arial",
        color: "#ffffff",
        align: "center",
    ...
    // Add subtitle
    this.add
      .text(400, 220, "Enter your name to begin:", {
        fontSize: "24px",
        fontFamily: "Arial",
        color: "#ffffff",
        align: "center",
    ...
    if (sceneData && sceneData.finalScore) {
      ...
      // Display final score
      this.add
        .text(400, 500, `Previous Score: ${finalScore}`, {
          fontSize: "32px",
          fontFamily: "Arial",
          color: "#ffff00",
          align: "center",
  ...
  async syncPlayerData(playerData: PlayerData): Promise<void> {
    // Get player data from API and write it to localStorage
    const apiUrl = `${API_URL}/players/${playerData.id}`;
    const response = await fetch(apiUrl);
    const receivedPlayerData = await response.json();
    localStorage.setItem("playerData", JSON.stringify(receivedPlayerData));
    console.log("Synced player data:", receivedPlayerData);
  }
}
