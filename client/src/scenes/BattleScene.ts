...
export default class BattleScene extends Phaser.Scene {
    ...
    init(data: BattleSceneData): void {
        ...
        // If we have a player ID, fetch fresh data from the API
        if (this.playerId) {
    ...
    initFromPlayerData(playerData: PlayerData): void {
        ...
        if (playerData.currentEnemyMaxHealth !== undefined) {
            ...
            console.log(`Using enemy max health from data: ${this.enemyConfig.maxHp}`);
        }
    }
    ...
    async fetchInitialPlayerData(playerId: string): Promise<void> {
        try {
            ...
            if (response.ok) {
                ...
                console.error('Failed to fetch initial player data:', await response.text());
            }
        } catch (error) {
    ...
    create(): void {
        ...
        // Create gold text
        this.goldText = this.add.text(400, 50, `Gold: ${this.gold}`, {
            fontSize: '20px',
            fontFamily: 'Arial',
            color: '#ffffff'
        }).setOrigin(0.5);
    ...
    createHealthBars(): void {
        ...
        // Enemy health text
        this.enemyHealthText = this.add.text(600, 130, `${this.enemyConfig.hp ?? 0}/${this.enemyConfig.maxHp ?? 0}`, {
            fontSize: '16px',
            fontFamily: 'Arial',
            color: '#ffffff'
        }).setOrigin(0.5);
    ...
    executeAttack(damage: number): void {
        ...
        this.tweens.add({
            ...
            onComplete: () => {
                ...
                this.enemy.play('slime-hit-anim');
        ...
        this.enemy.on('animationcomplete', (animation: Phaser.Animations.Animation) => {
            if (animation.key === 'slime-hit-anim' && this.enemyConfig.hp > 0) {
                this.enemy.play('slime-idle-anim');
            }
        });
    }
    ...
    updateEnemyHealthBar(): void {
        ...
        if (this.enemyHealthText && this.enemyHealthText.active) {
            this.enemyHealthText.setText(`${this.enemyConfig.hp}/${this.enemyConfig.maxHp}`);
        }
    }
    enemyDefeated(): void {
        ...
        this.time.delayedCall(3000, () => {
            ...
            if (this.playerId) {
                this.fetchPlayerState(this.playerId, () => {
                    this.continueToNextEnemy(victoryText, rewardText, nextEnemyText, nextDifficulty);
                });
            } else {
    ...
    async fetchPlayerState(playerId: string, callback: () => void): Promise<void> {
        try {
    ...
    continueToNextEnemy(victoryText: Phaser.GameObjects.Text, rewardText: Phaser.GameObjects.Text, nextEnemyText: Phaser.GameObjects.Text, nextDifficulty: string): void {
